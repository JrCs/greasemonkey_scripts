...
try {
    ...
    crcToNumber["4F17CCB5"] = -1
    crcToNumber["4E2080F5"] = 0
    crcToNumber["B13EE005"] = 1
    crcToNumber["5BF41AB9"] = 2
    crcToNumber["1A26C428"] = 3
    crcToNumber["A30DE187"] = 4
    crcToNumber["A71C7A43"] = 5
    crcToNumber["E45A9C31"] = 6
    crcToNumber["46AA49E1"] = 7
    crcToNumber["81F01724"] = 8
    crcToNumber["39B9DA69"] = 9
    ...
    function convertColor(image_data, threshold) {
        ...
        for (var i = 0, n = pix.length; i < n; i += 4) {
            var color = pix[i] > threshold && pix[i+1] > threshold && pix[i+2] > threshold && pix[i+3] > threshold ? 255 : 0;
            pix[i]     = color; // red
            pix[i + 1] = color; // green
            pix[i + 2] = color; // blue
            pix[i + 3] = 255;   // alpha
    ...
    function decodeGrid($grid) {
        ...
        for (y = 0; y < nbRows; y++) { // each row
            for (x = 0; x < nbCols; x++) { // each col
                ...
                var imageData = ctx.getImageData(cutX, cutY, img.width-(cutX*2), (img.height/2)-cutY-2);
                var pixels    = convertColor(imageData, 250); // return a buffer of pixels encoded in 32bit
                var imageCRC  = dec32ToHex(CRC32.buf(pixels));
                var number    = crcToNumber[imageCRC];

                if (debug) {
                    ctx.putImageData(imageData, cutX, cutY) // Fill canvas with the thresholded image
                    var $numberElement =
                            $("<span>")
                            .css({ 'color':'yellow', 'background-color':'black', 'font-family': '"Lucida Console", Monaco, monospace' })
                            .text(" row=" + y + "; col=" + x +
